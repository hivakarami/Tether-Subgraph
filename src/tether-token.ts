...
export function handleTransfer(event: TransferEvent): void {
  ...
  let token = Token.load(event.params.value.toString());
  if (!token) {
    token = new Token(event.params.value.toString());
    token.creator = event.params.to.toHexString();
    token.tokenID = event.params.value;
    token.createdAtTimestamp = event.block.timestamp;
 
    //let tokenContract = TokenContract.bind(event.address);
    //token.contentURI = tokenContract.tokenURI(event.params.value);
    //token.metadataURI = tokenContract.tokenMetadataURI(event.params.value);
  }
